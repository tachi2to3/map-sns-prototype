...
// ★追加：同じ場所にある投稿のスタック（重なり）アイコン
// 写真が重なっているようなデザイン + 件数バッジ
const StackMarker = ({ posts, onClick }) => {
  ...
  return (
    ...
    >
      ...
      {/* 件数バッジ */}
      <div className="absolute -top-2 -right-2 bg-[#Decbb7] text-[#1a1a1a] font-bold rounded-full w-6 h-6 flex items-center justify-center text-xs shadow-md border border-[#1a1a1a]">
        {count}
      </div>
    </div>
  );
          ...
          <div className="grid grid-cols-2 gap-3">
            {posts.map(post => (
              <div 
                ...
                className="relative aspect-square bg-[#1a1a1a] rounded-xl overflow-hidden border border-white/10 cursor-pointer group"
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    {post.userIcon ? (
                      <img src={post.userIcon} className="w-4 h-4 rounded-full" />
...
export default function Map() {
  ...
  // ★重要：投稿を「場所」ごとにグループ化するロジック
  // 座標を少し丸めてキーにすることで、ごく僅かなズレも吸収してグループ化します
  const groupedPosts = useMemo(() => {
    const groups = {};
    posts.forEach(post => {
      // 座標を小数点4位（約11m）で丸めてキーにする
      const key = `${post.lat.toFixed(4)},${post.lng.toFixed(4)}`;
      if (!groups[key]) groups[key] = [];
      groups[key].push(post);
    });
    return Object.values(groups); // 配列の配列 [[post1], [post2, post3], ...]
  }, [posts]);
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden bg-black">
      ...
      {/* ★追加: コレクションモーダル表示 */}
      {collectionModalPosts && (
        <CollectionModal 
          posts={collectionModalPosts} 
          onClose={() => setCollectionModalPosts(null)} 
          navigate={navigate} 
        />
      )}
      ...
      <GoogleMap
        ...
        options={{ disableDefaultUI: true, zoomControl: false, gestureHandling: "greedy", styles: mapStyles, backgroundColor: '#000000' }}
      >
        {currentLocation && (
                ...
                return (
                  <Marker
                    ...
                    onClick={() => {
                      ...
                      map.setZoom(16); 
                      // グループが複数件ならリストを開く、1件なら詳細へ
                      if (group.length > 1) {
                        setCollectionModalPosts(group);
                      } else {
              ...
              return (
                ...
                >
                  <div className="absolute transform -translate-x-1/2 -translate-y-1/2" style={{ zIndex: 50 }}>
                    <StackMarker 
                      posts={group} 
                      onClick={() => setCollectionModalPosts(group)} 
                    />
                  </div>
                </OverlayViewF>
              );
